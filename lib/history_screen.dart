...
class _AttendanceHistoryScreenState extends State<AttendanceHistoryScreen> {
  ...
  Future<String> _getEventNote(String date) async {
    DocumentSnapshot doc = await FirebaseFirestore.instance.collection('eventNotes').doc(date).get();
    if (doc.exists) {
      return doc['note'] ?? "";
    } else {
      return "";
  ...
  Future<void> _saveEventNote(String date, String note) async {
    await FirebaseFirestore.instance.collection('eventNotes').doc(date).set({'note': note});
  }
            ...
            return AlertDialog(
              ...
              actions: [
                TextButton(
                  ...
                  child: Text(isEditing ? "Renunță" : "Închide", style: const TextStyle(color: Colors.deepPurple)),
                ),
                TextButton(
                  onPressed: () async {
                    if (!isEditing) {
                  ...
                  child: Text(isEditing ? "Salvează" : "Edit", style: const TextStyle(color: Colors.deepPurple)),
                ),
              ],
            );
          },
  ...
  Future<void> _deleteEvent(BuildContext context, String date) async {
    ...
    bool? confirm = await showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
          title: const Text("Confirmare", style: TextStyle(color: Colors.deepPurple, fontWeight: FontWeight.bold)),
          content: const Text("Ești sigur că vrei să ștergi evenimentul?", style: TextStyle(color: Colors.deepPurple)),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(false),
              child: const Text("Anulează", style: TextStyle(color: Colors.deepPurple)),
            ),
            TextButton(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: FutureBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                return Text("Eroare: ${snapshot.error}", style: const TextStyle(color: Colors.deepPurple));
              }
              final presentMembers = snapshot.data ?? [];
              if (presentMembers.isEmpty) {
                return const Text("Niciun membru prezent.", style: TextStyle(color: Colors.deepPurple));
              }
              return SizedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return ListTile(
                      title: Text(presentMembers[index]['name'], style: const TextStyle(color: Colors.deepPurple)),
                    );
                  },
                ),
              );
          ...
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: const Text("Închide", style: TextStyle(color: Colors.deepPurple)),
            ),
            if (currentUserRole == UserRole.admin)
              TextButton(
                onPressed: () async {
                ...
                child: const Text("Șterge eveniment", style: TextStyle(color: Colors.red)),
              ),
          ],
        );
      },
  ...
  // Funcție de refresh pentru HistoryScreen
  void _refreshHistory() {
    setState(() {}); // Apelăm setState pentru a reîncărca FutureBuilder-ul
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: GlobalAppBar(title: "Istoric Prezențe", onRefresh: _refreshHistory),
      backgroundColor: Colors.white,
      body: FutureBuilder<Map<String, int>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Center(child: Text("Eroare: ${snapshot.error}", style: const TextStyle(color: Colors.deepPurple)));
          }
          final history = snapshot.data ?? {};
          final dates = history.keys.toList()..sort((a, b) => b.compareTo(a));
          if (dates.isEmpty) {
            return const Center(child: Text("Nu există prezențe înregistrate.", style: TextStyle(color: Colors.deepPurple)));
          }
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          IconButton(
                            icon: Icon(isExpanded ? Icons.keyboard_arrow_down : Icons.arrow_forward_ios, color: Colors.deepPurple),
                            onPressed: () {
                              setState(() {
                                if (isExpanded) {
                                  _expandedEvents.remove(date);
                        ...
                        FutureBuilder<String>(
                          ...
                          builder: (context, snapshot) {
                            if (snapshot.connectionState == ConnectionState.waiting) {
                              ...
                              return const Padding(
                                padding: EdgeInsets.only(top: 8.0),
                                child: Text("Eroare la încărcarea notiței", style: TextStyle(color: Colors.deepPurple)),
                              );
                            }
                            String note = snapshot.data ?? "";
                            return Padding(
                              padding: const EdgeInsets.only(top: 8.0),
                              child: Text(note.isNotEmpty ? note : "Fără notiță", style: const TextStyle(color: Colors.deepPurple, fontSize: 16)),
                            );
                          },
                        ),
                    ],
